...
def openCoachHomeContent(mainScreen, content, filepath):
    clearContent(mainScreen, content)
    coachHomeContent = CoachHomeScreenContent(mainScreen, filepath)
    coachHomeContent.generateCoachHomeScreenContnt(logins.finalloginname, mainScreen)


# Opens member details screen
def openMemberContent(mainScreen, content, filepath):
    clearContent(mainScreen, content)
    memberContent = MemberContent(mainScreen, filepath)
    memberContent.generateMemberContnt()


# Opens coaching session screen
def openCoachSessionContent(mainScreen, content, filepath):
    clearContent(mainScreen, content)
    myCoaching = CoachingSessionContent(mainScreen, filepath)
    myCoaching.coachSelection()
    myCoaching.generateCoachSessionContnt()


# Opens competition screen
def openNewCompetitionContent(mainScreen, content, filepath):
    clearContent(mainScreen, content)
    competitioncontent = NewCompetitionContent(mainScreen, filepath)
    competitioncontent.generateCompetitionContnt()


# Opens attend competitions screen
def openAttendCompetitionContent(mainScreen, content, filepath):
    clearContent(mainScreen, content)
    attendingcontent = AttendingContent(mainScreen, filepath)
    attendingcontent.generateAttendingContnt()


# Opens coach reports screen
def openReportsContent(mainScreen, content, filepath):
    clearContent(mainScreen, content)
    coachreportcontent = CoachReportsContent(mainScreen, filepath)
...
def main(filepath):
    ...
    CoachHomeButton = Button(header, cursor="tcross", image=coachhomephoto, width=30, height=30, command=lambda : openCoachHomeContent(mainScreen, content, filepath), bg="black",bd=4,relief='ridge')
    ...
    add_member_button = tkinter.Button(header, text="Member", command=lambda : openMemberContent(mainScreen, content, filepath), fg='white', bg='black', bd=4, relief='ridge', font=('Tahoma', 12, 'bold'), padx=10, cursor="tcross")
    add_member_button.place(rely=0.5, relx=0.155, anchor='center')
    ToolTips.bind(add_member_button, 'Member Screen')

    coaching_session_button = tkinter.Button(header, text="Coaching Session", command=lambda : openCoachSessionContent(mainScreen, content, filepath), fg='white', bg='black', bd=4, relief='ridge', font=('Tahoma', 12, 'bold'), padx=10, cursor="tcross")
    ...
    new_competition_button = tkinter.Button(header, text="Competition", command=lambda : openNewCompetitionContent(mainScreen, content, filepath), fg='white', bg='black', bd=4, relief='ridge', font=('Tahoma', 12, 'bold'), padx=10, cursor="tcross")
    new_competition_button.place(rely=0.5, relx=0.491, anchor='center')
    ToolTips.bind(new_competition_button, 'Competition Screen')

    attend_competition_button = tkinter.Button(header, text="Attend Competition", command=lambda : openAttendCompetitionContent(mainScreen, content, filepath), fg='white', bg='black', bd=4, relief='ridge', font=('Tahoma', 12, 'bold'), padx=10, cursor="tcross")
    ...
    reports_button = tkinter.Button(header, text="Reports", command=lambda : openReportsContent(mainScreen, content, filepath), fg='white', bg='black', bd=4, relief='ridge', font=('Tahoma', 12, 'bold'), padx=10, cursor="tcross")
